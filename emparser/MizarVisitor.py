...
class MizarVisitor(ParseTreeVisitor):
    ...
    def visitVocabulary_directive(self, ctx:MizarParser.Vocabulary_directiveContext):
        vocabularies = ET.Element('vocabularies')
        for child in self.visitChildren(ctx):
            ET.SubElement(vocabularies, child)
        return vocabularies
    ...
    def visitVocabulary_name(self, ctx:MizarParser.Vocabulary_nameContext):
        ...
        filename.text = ctx.FILE_NAME().getText()
        return filename
    ...
    def visitTheorem(self, ctx:MizarParser.TheoremContext):
        theorem = ET.Element('theorem')
        for child in self.visitChildren(ctx):
            ET.SubElement(theorem, child)
        return theorem
